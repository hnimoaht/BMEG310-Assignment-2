...
```{r}
    # fit logistic regression model on top 5 PCs
    logit.model.pca <- glm(diagnosis ~ ., data = pca.data[train.indices, c(1:5, ncol(pca.data))], family = binomial)

    # make predictions on training set
    train.probs.pca <- predict(logit.model.pca, type = "response")
    train.pred.pca <- ifelse(train.probs.pca > 0.5, "M", "B")

    # confusion matrix for training set
    train.confusion.pca <- table(Predicted = train.pred.pca, Actual = pca.data[train.indices, "diagnosis"])

    # calculate accuracy, precision, recall for training set
    train.accuracy.pca <- sum(diag(train.confusion.pca)) / sum(train.confusion.pca)
    train.precision.pca <- train.confusion.pca["M", "M"] / sum(train.confusion.pca["M", ])
    train.recall.pca <- train.confusion.pca["M", "M"] / sum(train.confusion.pca[, "M"])

    # make predictions on test set
    test.probs.pca <- predict(logit.model.pca, newdata = pca.data[test.indices, c(1:5, ncol(pca.data))], type = "response")
    test.pred.pca <- ifelse(test.probs.pca > 0.5, "M", "B")

    # confusion matrix for test set
    test.confusion.pca <- table(Predicted = test.pred.pca, Actual = pca.data[test.indices, "diagnosis"])

    # calculate accuracy, precision, recall for test set
    test.accuracy.pca <- sum(diag(test.confusion.pca)) / sum(test.confusion.pca)
    test.precision.pca <- test.confusion.pca["M", "M"] / sum(test.confusion.pca["M", ])
    test.recall.pca <- test.confusion.pca["M", "M"] / sum(test.confusion.pca[, "M"])

    # output results
    list(
        Train = list(Accuracy = train.accuracy.pca, Precision = train.precision.pca, Recall = train.recall.pca),
        Test = list(Accuracy = test.accuracy.pca, Precision = test.precision.pca, Recall = test.recall.pca)
    )
```
...
```{r}
    ...
    pred <- prediction(test.probs, ovarian.dataset.test$diagnosis)
    ...
    par(mfrow = c(1, 1))  # ensures ROC plot has full space

    # calculate AUC
    auc <- performance(pred, measure = "auc")
    auc.value <- auc@y.value[[1]]
    auc.value
```
